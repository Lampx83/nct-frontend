  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        // Chỉ lấy các category có slug trong danh sách mong muốn
        ...
        const mappedEvents = validCategories.flatMap((category) =>
          category.attributes.blogs.data.map((blog) => {
            ...
            return {
              id: blog.id,
              date: formattedDate, // Định dạng tiếng Việt
              category: category.attributes.title,
              title: blog.attributes.title,
              time: "N/A", // Nếu API không cung cấp giờ, để mặc định
  ...
  return (
    <div className="container">
      ...
      <div className="more-news text-end fs-5 my-0 py-0">
        <Link href="/tin-tuc" style={{ fontFamily: "Barlow, sans-serif" }} className="text-primary">Xem Thêm</Link>
      </div>

      {/* CSS giữ nguyên như code gốc của bạn */}
      <style jsx>{`
        .events-container {
          max-width: 1200px;
          margin: 0 auto;
